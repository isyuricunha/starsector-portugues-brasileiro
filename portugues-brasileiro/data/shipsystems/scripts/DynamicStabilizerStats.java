...
//		}
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		...
		if (index == 0) {
			return new StatusData("manobrabilidade aprimorada", false);
		} else if (index == 1) {
			return new StatusData("taxa de disparo +" + (int) rof + "%", false);
		} else if (index == 2) {
			return new StatusData("recuo -" + (int) recoil + "%", false);
		} else if (index == 3) {
			return new StatusData("dano do feixe +" + (int) beam + "%", false);
		}
