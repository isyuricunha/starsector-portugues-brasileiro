...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "'Só um comerciante honesto tentando ganhar a vida, oficial.'");
		api.setFleetTagline(FleetSide.ENEMY, "O 'Alto Rad' Moon Salazar, um trapaceiro de duas caras, em um cargueiro enferrujado");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("Mostre ao 'Alto Rad' Moon o que significa quebrar um acordo");
		api.addBriefingItem("Não perca o 'Stranger II' — é minha posse mais valiosa");
		api.addBriefingItem("A nave de Moon tem mods delicados no motor; use Mísseis Salamandra para deixá-la à deriva");
		...
		if (!testMode) {
			...
			api.addToFleet(FleetSide.PLAYER, "hound_d_Standard", FleetMemberType.SHIP, "Corrida de Leite", false);
			
			// Set up the enemy fleet.
			api.addToFleet(FleetSide.ENEMY, "mule_d_pirates_Smuggler", FleetMemberType.SHIP, "Florescência de Cherenkov", false);
...
//			member.getCaptain().getStats().setSkillLevel(Skills.HELMSMANSHIP, 2);
			...
			api.addToFleet(FleetSide.PLAYER, "falcon_Attack", FleetMemberType.SHIP, "Estranho II", true);
	//		PersonAPI person = new AICoreOfficerPluginImpl().createPerson(Commodities.ALPHA_CORE, null, null);
	//		member.setCaptain(person);
			
			api.addToFleet(FleetSide.ENEMY, "lasher_CS", FleetMemberType.SHIP, "Brilho de Cherenkov", false);
