...
public class MissionDefinition implements MissionDefinitionPlugin {
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		api.setFleetTagline(FleetSide.PLAYER, "Frota de alívio da Hegemonia com escolta mercenária");
		api.setFleetTagline(FleetSide.ENEMY, "Força-tarefa de contenção da Tri-Tachyon");
		
		// These show up as items in the bulleted list under 
		// "Tactical Objectives" on the mission detail screen
		api.addBriefingItem("A Estrela Negra da ISS deve sobreviver");
		api.addBriefingItem("Pelo menos 25% das forças da Hegemonia devem escapar");
		
		// Set up the player's fleet
		api.addToFleet(FleetSide.PLAYER, "hammerhead_Elite", FleetMemberType.SHIP, "Estrela Negra da ISS", true);
		api.addToFleet(FleetSide.PLAYER, "dominator_Outdated", FleetMemberType.SHIP, "HSS Tremor", false).getCaptain().setPersonality(Personalities.AGGRESSIVE);
		api.addToFleet(FleetSide.PLAYER, "monitor_Escort", FleetMemberType.SHIP, "HSS Áspide", false).getCaptain().setPersonality(Personalities.CAUTIOUS);
		api.addToFleet(FleetSide.PLAYER, "monitor_Escort", FleetMemberType.SHIP, "HSS Egide", false).getCaptain().setPersonality(Personalities.CAUTIOUS);
		api.addToFleet(FleetSide.PLAYER, "buffalo2_FS", FleetMemberType.SHIP, "HSS Arqueiro", false).getCaptain().setPersonality(Personalities.TIMID);
		api.addToFleet(FleetSide.PLAYER, "kite_hegemony_Interceptor", FleetMemberType.SHIP, "HSS Mosquito", false).getCaptain().setPersonality(Personalities.CAUTIOUS);
		...
		api.defeatOnShipLoss("Estrela Negra da ISS");
		...
		api.addToFleet(FleetSide.ENEMY, "medusa_Attack", FleetMemberType.SHIP, "TTS Euríale", false);
		api.addToFleet(FleetSide.ENEMY, "medusa_Attack", FleetMemberType.SHIP, "TTS Euríale", false);
