...
public class Magec {
	...
	public void generate(SectorAPI sector) {
		...
		SectorEntityToken achaman_buoy = system.addCustomEntity("achaman_relay", // unique id
				 "Rele de Achaman", // name - if null, defaultName from custom_entities.json will be used
		...
		magec3a.setCustomDescriptionId("planet_tibicena");
			...
			SectorEntityToken achaman_station = system.addCustomEntity("achaman_enterprise_station", 
																		"Estação Empresarial de Achaman",
																		"station_side04",
																		"tritachyon");
		...
		SectorEntityToken ring = system.addTerrain(Terrain.RING, new RingParams(600 + 256, 3500, null, "Disco de Guayota"));
		...
		// add one ring that covers all of the above
		ring = system.addTerrain(Terrain.RING, new RingParams(200 + 256, 4100, null, "Disco de Guayota"));
		ring.setCircularOrbit(star, 0, 0, 100);
...
//		system.addRingBand(a2, "misc", "rings1", 256f, 1, Color.white, 256f, 1700, 110f);
		...
		// add one ring that covers all of the above
		ring = system.addTerrain(Terrain.RING, new RingParams(200 + 256, 4400, null, "Disco de Guayota"));
		ring.setCircularOrbit(star, 0, 0, 100);
		...
		// add one ring that covers all of the above
		ring = system.addTerrain(Terrain.RING, new RingParams(300 + 256, 4650, null, "Disco de Guayota"));
		ring.setCircularOrbit(star, 0, 0, 100);
		...
		SectorEntityToken guayota_relay = system.addCustomEntity("guayota_relay", // unique id
				 "Rele de Guayota", // name - if null, defaultName from custom_entities.json will be used
		...
		SectorEntityToken pirateStation = system.addCustomEntity("kantas_den", "Covil de Kanta", "station_side06", "pirates");
		...
		JumpPointAPI jumpPoint = Global.getFactory().createJumpPoint("maxios_jump_point", "Ponto de Salto de Maxios");
		...
		SectorEntityToken gate = system.addCustomEntity("magec_gate", // unique id
				 "Portal de Magec", // name - if null, defaultName from custom_entities.json will be used
